...
def cb():
    ...
    def role():
        ...
        if len(main_team_list) > 0:
            types_cb = ["center back", "ball playing center back", "wide center back", "no non-sense center back"]
            
            ### no wide center backs for formations with 4 defenders or a sweeper
            if cwp == 1 or defender == 4:
                types_cb.remove("wide center back")
                role = random.choice(types_cb)
            ### 2 wide center backs for formations with 3 defenders without a sweeper
            elif cb == 3 and "wide center back" in types_cb3:
                types_cb3.remove("wide center back")
                role = "wide center back"
            elif cb == 3 and "wide center back" not in types_cb3:
                role = random.choice(types_cb3)
            else:
                role = random.choice(types_cb)
            return role
        else:
            sub_cb_roles = ["center back", "ball playing center back", "wide center back", "no non-sense center back"]
    ...
    BALANCE = (skill_role_cb[player_role] + skill_pos[position]) / 2 + str_fac
    ...
    def second_pos():
        def choices(y,z):
            ...
            return x
        roles = ["center back", "ball playing center back", "wide center back", "no non-sense center back"]
